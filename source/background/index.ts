...
import {ExtensionMessage, isLogMessage, isOAuthLaunchMessage, OAuthLaunchResponse} from '../types/messages';
import type {NativeOAuthRequest, NativeOAuthResponse} from '../types/native-messages';
...
browser.runtime.onMessage.addListener(async (message: unknown): Promise<ExtensionMessage | undefined> => {
  ...
  if (isOAuthLaunchMessage(message)) {
    ...
    try {
      const _nativeAppResponse = await browser.runtime.sendNativeMessage(SAFARI_NATIVE_APP_NAME, {
        type: 'oauth',
      ...
      log.background.info('OAuth  native succeeded');
      return {
        redirectURL: nativeAppResponse.redirectURL,
      } satisfies OAuthLaunchResponse;
    } catch (err) {
      log.background.error('OAuth native message failed', err instanceof Error ? err : String(err));
